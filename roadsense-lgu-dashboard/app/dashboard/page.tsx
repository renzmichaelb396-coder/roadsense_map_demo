...
const STATUS_DIM_STYLE = "opacity:0.25;";
...
function createLGUMarker(h: Hazard) {
  ...
  el.style.cssText = `
    ...
    height:40px;
    border-radius:999px;
    background:${SEVERITY_COLOR[h.severity as Severity]};
    color:#ffffff;
    font-weight:900;
    font-size:18px;
    display:flex;
...
function exportToCSV(visibleHazards: Hazard[]) {
  const headers = [
    "type",
    "severity",
    "status",
          ...
          marker.setPopup(
            new mapboxgl.Popup({ offset: 16 }).setHTML(`
              <div style="background:#0b0b0b;color:#fff;padding:14px;border-radius:12px">
                ...
                <span style="color:${SEVERITY_COLOR[h.severity as Severity]}">${h.severity}</span><br/>
...
if (statusFilter === "ACTIVE" && h.status === "resolved") {
  marker.getElement().style.cssText += STATUS_DIM_STYLE;
}
if (statusFilter === "RESOLVED" && h.status !== "resolved") {
  marker.getElement().style.cssText += STATUS_DIM_STYLE;
}
