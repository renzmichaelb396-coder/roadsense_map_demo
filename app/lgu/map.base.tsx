...
async function safeReloadHazards() {
  try {
    ...
    const normalized = arr.map((h:any) => ({
      ...h,
      ...
      severity:
        h?.severity === "HIGH" ? 3 :
        h?.severity === "MEDIUM" ? 2 :
        h?.severity === "LOW" ? 1 : h?.severity,
    }));
    setHazards(normalized as any);
  } catch (e) {
...
const visibleHazards = useMemo(() => {
  return hazardsStable.filter((h: any) => {
    const st = safeStatus(h);    if (!hasValidCoord(h)) return false;
...
}, [hazardsStable, severityFilter, typeFilter, showResolved]);const counts = useMemo(() => {
